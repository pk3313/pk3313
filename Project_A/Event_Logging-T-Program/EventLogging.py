...
class WindowClass(QMainWindow,QWidget, form_class):
    ...
    def eventSave(self):
        ...
        if len(str(self.OutputData_Event)) != 0:
            self.eventOutput()
            self.saveText()
            self.saveExcel()
...
class RECVTHREAD(QThread):
    ...
    def setNaviData(self):
        ...
        NaviData.LatBrowser.setText(str(INGLL.rov_UTM_X))
        NaviData.LonBrowser.setText(str(INGLL.rov_UTM_Y))
        ...
        NaviData.ListNaviData[1] = str(INGLL.rov_UTM_X)
        NaviData.ListNaviData[2] = str(INGLL.rov_UTM_Y)        
